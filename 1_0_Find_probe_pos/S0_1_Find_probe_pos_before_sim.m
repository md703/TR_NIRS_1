...
%% param
model_folder='models';
subject_name_arr={'KB'};  % {'KB','BY','CT','TY','CS'}
...
if do_plot_3D
    ...
    for s=1:size(p_pos,1)
        ...
        elseif s<=7
            ...
            quiver3(p_pos(s,1),p_pos(s,2),p_pos(s,3),p_dir(s,1)*dir_len,p_dir(s,2)*dir_len,p_dir(s,3)*dir_len,'b','LineWidth',3,'MarkerSize',24);
            if do_SDS_text
                text(p_pos(s,1)+p_dir(s,1)*dir_len,p_pos(s,2)+p_dir(s,2)*dir_len,p_pos(s,3)+p_dir(s,3)*dir_len,['SDS ' num2str(s-1)],'FontSize',15,'FontName','Times New Roman','Color','g');
            end
        end
