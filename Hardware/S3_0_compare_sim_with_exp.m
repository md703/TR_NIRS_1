...
%% for binning calibration (fail)

% folderPath='20240502';
% sim_folder='cal_reflectance_10';
% 
% fitting_phantom=[1];
% fitting_SDS=[1 2];
% 
% 
% for p=1:6
%     sim_TPSF(:,:,p)=load(fullfile(folderPath,sim_folder,['phantom_' num2str(p) '_TPSF.txt']));
% end
% 
% for p=fitting_phantom
%     
...
%     ti=tiledlayout('flow');
%     
%     IRF=load(fullfile(folderPath,['phantom_' num2str(p)],'IRF_TPSF.txt'));
%     
%     exp_TPSF(:,:,p)=load(fullfile(folderPath,['phantom_' num2str(p)],'SDS_TPSF.txt'));
%     for s=fitting_SDS
%         temp_sim_TPSF=sim_TPSF(:,s,p);
%         temp_exp_TPSF=exp_TPSF(:,s,p);
%         
%         calib_sim_TPSF=conv(temp_sim_TPSF,IRF);
%         
%         max_value=max(temp_sim_TPSF);
%         temp_sim_TPSF=temp_sim_TPSF./max_value;
%         
