...
Also add the noise to the simultion spectrum
...
%% param
subject_name_arr={'KB','BY','CT'};%,'WH','WW'
...
SDS_error_arr=[2 2.3 0.75 4 1 2]; % in %, the error (CV) of each SDS (noise for continuous-wave)
% SDS_error_arr=[3 4.2 5.1 5.2 5.4 12.1]; % in %, the error (CV) of each SDS

load('../1_2_Calculate_cv/results/cv_value_1E11.mat'); % cv_value (simulation noise for time-resolved)
...
for sbj=1:length(subject_name_arr)
    ...
    for target_i=1:num_anser_to_generate
        ...
        % generate original spectral and DTOFs
