...
%% param
input_dir='test_fitting_2024-06-16-00-59-47'; % please move the fitting folders into this folder first.
...
fitting_dir='fitting_tr1234_gate123456'; % the fitting folder
...
%                 fprintf(fid,'\n');
        ...
        %                 close all;
                ...
                if tr_flag
                    fig=figure('Units','pixels','position',[0 0 1080 500]);
                    ...
                    fitting_op_tr=squeeze(interp1(lambda,fitting_op_arr,fitting_wl_tr));
                    OP_answer_tr=interp1(lambda,OP_answer_arr(:,:,target_i),fitting_wl_tr);
                    
                    nexttile;
                    plot([1 2 3],OP_answer_tr([1 3 7]),'--o','Color',[0 0.4470 0.7410]);
                    hold on
                    for j=times_to_fitting:-1:1
                        plot([1 2 3],squeeze(fitting_op_tr([1 3 7],error_index(j))),'.','Color',colormap_arr(j,:),'MarkerSize',20);
                    end
                    xticks(1:1:3);
                    xticklabels(op_name(1:3));
                    ylabel('(1/cm)');
                    grid on
                    set(gca,'fontsize',14, 'FontName', 'Times New Roman');
                    
                    nexttile;
                    plot([1 2 3],OP_answer_tr([2 4 8]),'--o','Color',[0 0.4470 0.7410]);
                    hold on
                    for j=times_to_fitting:-1:1
                        plot([1 2 3],squeeze(fitting_op_tr([2 4 8],error_index(j))),'.','Color',colormap_arr(j,:),'MarkerSize',20);
                    end
                    xticks(1:1:3);
                    xticklabels(op_name(4:6));
                    ylabel('(1/cm)');
                    grid on
                    set(gca,'fontsize',14, 'FontName', 'Times New Roman');

                    lgd=legend(legend_arr,'fontsize',14, 'FontName', 'Times New Roman');
                    lgd.NumColumns=lgdNumCol;
                    lgd.Layout.Tile = 'south';

                    print(fullfile(temp_target_folder,'fitted_mu_TR.png'),'-dpng','-r200');
        %                 close all;
                end
...
%                 h.YDisplayData=flipud(h.YDisplayData);
            ...
            if do_plot_anyPlot
                ...
                fig=figure('Units','pixels','position',[0 0 720 360]);
                set(fig,'visible','off');
                ti=tiledlayout('flow','TileSpacing','compact','Padding','none');
                % fitted error and init error
                nexttile();
                plot(init_param_arr(:,end),fitting_result(:,num_fitted_param+num_SDS_cw+num_SDS_tr+3),'o','Linewidth',2);
                grid on;
                xlabel('init error');
                ylabel('fitted error');
                ...
                % OP error
                nexttile();
                plot(fitting_result(:,num_fitted_param+num_SDS_cw+num_SDS_tr+3),fitting_result(:,num_fitted_param+num_SDS_cw+num_SDS_tr+3+7),'o','Linewidth',2);
                grid on;
                xlabel('DTOF error');
                ylabel('RMSPE of all OPs');
                yylim=ylim(); yylim(1)=0; ylim(yylim);
                set(gca,'fontsize',12, 'FontName', 'Times New Roman');
...
%                         set(fig,'visible','off');
                        ...
                        print(fullfile(temp_target_folder,['fitted_dtof_r' num2str(jj) '_f' num2str(j) '.png']),'-dpng','-r200');
    %                     close all;
                    end
                end
            end
