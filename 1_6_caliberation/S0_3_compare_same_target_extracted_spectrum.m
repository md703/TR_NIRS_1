%{
Compare the extracted (minus BG) spectrum of the same target or BG, to see if the spectrum are different
...
%% param
input_dir='20201215_test_17'; % the folder of the experiment
extracted_folder='extracted_spec'; % the folder of the extracted spectrum
target_name_prefix='kb_'; % the name of the target to plot
target_index=[5:7]; % the index to plot
output_name='Compare_extracted_kb_5_7'; % the saved picture name
% output_name='takeUp_tc'; % the saved picture name
reset_legend_name=1; % =1 to rename the index to [1 2 ......]
reset_subject_name='subject 5';
...
%         legend_arr{i}=[strrep(target_name_prefix,'_',' ') num2str(i)];
        legend_arr{i}=[reset_subject_name '_{' num2str(i) '}'];
    else
...
BG_spec_arr={};
for i=1:length(target_name_arr)
    temp=load(fullfile(input_dir,extracted_folder,[target_name_arr{i} '.txt']));
    if i==1
        plot_wl=temp(:,1);
    end
    for s=1:num_SDS
...
for s=1:num_SDS
    ...
    hold on;
    for i=1:length(target_name_arr)
        plot(plot_wl,BG_spec_arr{s}(i,:),'Color',color_arr(i,:),'LineWidth',lineWidth);
    end
    xlabel('wavelength');
    ylabel('Reflectance');
    
    yyaxis right;
    plot(plot_wl,abs(CV_spec(s,:))*100,'--','LineWidth',lineWidth);
    ylabel('CV(%)');
    yylim=ylim;
