...
for fi=1:length(folder_arr)
    ...
    for wl=1:length(wl_interval)
        ...
        for s=1:num_SDS
            ...
            for j=1:(num_phantom+1)
                ...
                if figure_monitoring && j==num_phantom+1 && rem(wl,monitoring_interval)==1
                    ...
                    text(mid_x_point,interp1(x_arr,pred_y_arr{num_phantom+1},mid_x_point)*0.9,{['y = ' num2str(A_arr(wl,s,j),'%.2e') ' \times x + ' num2str(B_arr(wl,s,j),'%.2e')],['r^2 = ' num2str(RR_arr(wl,s,j),'%.4f')]},'HorizontalAlignment','left','VerticalAlignment','top','fontsize',fontSize, 'FontName', 'Times New Roman');
        ...
        if figure_monitoring && rem(wl,monitoring_interval)==1
            ...
            text(((left_spacing+subplot_width)*plot_n_col+right_spacing)/2,(upper_spacing+subplot_height)*plot_n_row+legend_height+upper_spacing/2+lower_spacing,[calib_mode_arr{calib_mode} ' calibration, wl ' num2str(wl_interval(wl)) ' nm'],'Unit','pixels','HorizontalAlignment','center','VerticalAlignment','middle','fontsize',fontSize,'FontName', 'Times New Roman')
