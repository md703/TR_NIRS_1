...
This code generate more results of different mus-set combinations through interpolation.
...
% about random
num_random=1000; % how many number to random, for mus.
...
for sbj_i=1:length(subject_name_arr)
    ...
    %% test
    if test_mode==1
        layer_mus={225,200,23,[25:10:275]};
        mus_param_arr=[];
    ...
    for mua_i=1:size(mua_param_arr,1)
        fprintf('Calculating mua %d/%d',mua_i,size(mua_param_arr,1)); %, Gate 
        for s=1:num_total
%             fprintf('%d ',s);
            if test_mode==0
