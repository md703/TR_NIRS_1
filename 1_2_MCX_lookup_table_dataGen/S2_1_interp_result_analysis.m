%{
Plot reflectance before and after interpolation
...
interp_dir_arr={'CT_test1_2024-07-08-13-08-21'};
...
for sbj_i=1:length(subject_name_arr)
    ...
    ti=tiledlayout(5,10,'TileSpacing','compact','Padding','none');
    for s=1:num_SDS
        for g=1:num_gate
            nexttile;
            plot(param_arr(:,8),param_arr(:,8+g+(s-1)*num_gate),'-o','Linewidth',2);
            hold on
            plot(param_arr_interp(:,8),param_arr_interp(:,8+g+(s-1)*num_gate),'Linewidth',2);
            title(['SDS ' num2str(s) ' Gate ' num2str(g)],'FontWeight','Normal');
            xlabel('\mu_{s,GM}');
            ylabel('reflectance');
%             set(gca,'FontSize',20);
        end
