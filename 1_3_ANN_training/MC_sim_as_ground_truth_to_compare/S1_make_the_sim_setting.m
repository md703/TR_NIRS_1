...
%% init
% make the mus table
load(['../model_arrange/' target_name_arr{1} '_tr_model']) %tr_net,tr_param_range

mu_table=[];
for t=1:15
    mu_table(end+1,:)=rand(1,8).*(tr_param_range(1,:)-tr_param_range(2,:))+tr_param_range(2,:);
end
...
for ti=1:length(target_name_arr)
    clearvars -except target_name_arr ti model_folder layer_mus mus_table mu_table sim_set sim_version n g;
